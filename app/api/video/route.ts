...
export async function POST(req: NextRequest) {
  try {
    if (!process.env.RUNWAYML_API_SECRET) {
      return NextResponse.json({ error: "服务器尚未配置 RUNWAYML_API_SECRET" }, { status: 500 });
    }
    ...
    if (!ratios[ratio]) return NextResponse.json({ error: "不支持的画幅" }, { status: 400 });
...
export async function GET(req: NextRequest) {
  try {
    ...
    console.error(error);
    return NextResponse.json({ error: error instanceof Error ? error.message : "查询任务失败" }, { status: 500 });
  }
}
